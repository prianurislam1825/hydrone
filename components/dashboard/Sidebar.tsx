...
export default function Sidebar() {
  ...
  const userEmail = session?.user?.email || 'admin@hydrone.id'
  const userImage = session?.user?.image
  const initial   = userName.charAt(0).toUpperCase()

  return (
    <aside
      className="hidden lg:flex flex-col shrink-0 border-r transition-all duration-300 relative sticky top-0 h-screen z-30"
      style={{
    ...
    >
      <div className="flex flex-col h-full w-full overflow-y-auto overflow-x-hidden">
        ...
        {/* ── Bottom: user + logout ─────────────────────── */}
        <div className="border-t px-2 py-3 flex flex-col gap-1 mt-auto" style={{ borderColor: 'var(--t-border)' }}>
          {!collapsed && (
            <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-xl min-w-0"
              style={{ background: 'rgba(26,86,219,0.05)' }}>
              <div className="w-7 h-7 rounded-full bg-[#1A56DB] flex items-center justify-center shrink-0 overflow-hidden relative border border-white/20">
                {userImage ? (
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs font-bold truncate" style={{ color: 'var(--t-text)' }}>{userName}</div>
                <div className="text-[10px] truncate" style={{ color: 'var(--t-muted)' }}>{userEmail}</div>
              </div>
            </div>
          )}
          <Link href="/login"
      ...
      {/* ── Collapse toggle (outside scroll container) ───────────────── */}
      <button
        onClick={() => setCollapsed(v => !v)}
        className="absolute -right-3 top-16 w-6 h-6 rounded-full border flex items-center justify-center transition-all hover:border-[#1A56DB] shadow-md z-40"
